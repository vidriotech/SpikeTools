@testset "Import KiloSort annotations from .npy files" begin
    ksannotation = loadkilosort(ENV["DATADIR"])

    @test nclusters(ksannotation) == 358
    @test nspikes(ksannotation) == 9014015
    @test length(ProbeTools.channelmap(probe(ksannotation))) == 374
    @test size(ProbeTools.channelpositions(probe(ksannotation))) == (374, 2)
